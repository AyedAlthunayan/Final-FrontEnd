...
function Login() {
    ...
    const handleSubmit = async () => {
        axios.post(APIs.USER.LOGIN, user).then(res => {
            Swal.fire({
                icon: 'success',
                    ...
                    <HStack spacing='10'>
                        <Link to=''>
                            <Button onClick={(e) => handleSubmit(e)} color='white'
                                    ...
                                    backgroundColor='black'>ارسال</Button>
                        </Link>
                        <Link to='/register'>
                            <Button color='white' _hover={{backgroundColor: '#FFD05E', color: 'black'}}
                                    backgroundColor='black'>لتسجيل حساب جديد</Button>
                        </Link>
                    </HStack>
